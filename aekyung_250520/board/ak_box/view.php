...
// 2014.05.14 로그인 이후 볼 수 있는 기능 제거
...
<div class="contents_area">
	<div class="page_title" style="padding: 0;margin: 0;border: 0">
		<h2>
		...
		<dl>
			<dt style="width: 100%; text-align: center">
				<a
					...
					class="bg1" style='margin: 10px'>애경박스신청 확인</span></a> <a
					href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_05&page=<?=$_GET['page']?>&idx=<?=$_GET['idx']?>"><span
...
<div id="content_wrap" style="padding:0;">
		<!--<div class="spm_txt">
			<dl>
...
<?
	if($out_row['hero_thumb']){
		$img_new = $out_row['hero_thumb'];
	}else{
		$img_new = $out_row['hero_img_new'];
	}
?>
				<img src='<?=$out_row['hero_img_new']?>' width='289px' height='228px'>
			</div>
			<div id="content_top">
				<table>
					<tr>
						<td class='cate'>참여방법</td>
						<td class='cont' height="auto"><?=$out_row['hero_help_01']?></td>
					</tr>
					<tr>
						<td class='cate'>참여대상</td>
						<td class='cont' height="auto"><?=$out_row['hero_target']?></td>
					</tr>
					<tr>
						<td class='cate'>미션신청기간</td>
						
						<td class='cont' height="auto"><?=date('m월 d일',strtotime($out_row['hero_today_01_01'])).'('.$startD.')'.' - '.date('m월 d일',strtotime($out_row['hero_today_01_02'])).'('.$startD.')'?></td>
					</tr>
					<tr>
						<td class='cate'>당첨자발표</td>
						<td class='cont' height="auto"><?=date('m월 d일',strtotime($out_row['hero_today_02_01'])).'('.$releaseSD.')'?></td>
					</tr>
					<tr>
						<td class='cate'>리뷰등록기간</td>
						<td class='cont' height="auto"><?=date('m월 d일',strtotime($out_row['hero_today_03_01'])).'('.$reviewSD.')'.' - '.date('m월 d일',strtotime($out_row['hero_today_03_02'])).'('.$reviewED.')'?></td>
					</tr>
					<tr>
						<td class='cate'>미션혜택</td>
						<td class='cont' height="auto"><?=$out_row['hero_benefit']?></td>
					</tr>
					<tr style='border-bottom:0px;'>
						<td class='cate'>필수미션</td>
			...
			<div class="spm_img" style="padding:0;"><?=htmlspecialchars_decode($out_row['hero_command']);?></div>
...
?>
	...
	<table>
		<tr>
			<td class='cate'>미션 신청 기간</td>
			<td class='cont'>
				<?=date ( "Y년 m월 d일", strtotime ($out_row ['hero_today_01_01'])).'('.$startD.')'.' - '.date ( "m월 d일", strtotime ($out_row ['hero_today_01_02'])).'('.$endD.')';?>
			</td>
		</tr>
		<tr>
			<td class='cate'>당첨자 발표</td>
			<td class='cont'>
				<?=date ( "Y년 m월 d일", strtotime ($out_row ['hero_today_02_01'])).'('.$releaseSD.')';?>
			</td>
		</tr>
		<tr>
			<td class='cate'>당첨자 혜택</td>
			<td class='cont'>
				<?=$out_row ['hero_benefit'];?>
			</td>
		</tr>
	<?if($out_row ['hero_benefit_02']!=''){?>
		<tr>
			<td class='cate'>우수자 혜택</td>
			<td class='cont'>
				<?=$out_row ['hero_benefit_02'];?>
			</td>
		</tr>
	<?}?>
		<tr>
			<td class='cate'>미션 가이드</td>
			<td class='cont'>
	...
	<?}?>
		<tr>
			<td class='cate'>리뷰 등록 기간</td>
			<td class='cont'>
				<?=date ( "Y년 m월 d일", strtotime ($out_row ['hero_today_03_01'])).'('.$reviewSD.')'.' - '.date ( "m월 d일", strtotime ($out_row ['hero_today_03_02'])).'('.$reviewED.')';?>
			</td>
		</tr>
		<tr>
			<td class='cate'>미션 참여 안내</td>
			<td class='cont' style='padding-bottom: 11px;'>
				<?=str_replace("\n","<br>",$out_row['hero_guide']);?>
			</td>
		</tr>
		<tr>
			<td class='cate'>필수 태그</td>
			<td class='cont' style='padding-bottom: 11px;'>
				<?=$out_row ['hero_tag'];?>
			</td>
		</tr>
		<tr>
			...
			<td class='cont' style='padding-bottom: 11px;'>
				<?=htmlspecialchars($out_row ['hero_banner']);?>
			</td>
		</tr>
	</table>
...
if (! strcmp ( $setup_type, '1' )) {
	if (! strcmp ( $data_count, '0' )) {
		if (strcmp ( $_SESSION ['temp_id'], '' )) {
			?>
            <a
					href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_01&idx=<?=$out_row['hero_idx']?>"><img
					src="/image2/etc/akbox_button.png" alt="참여하기" style='margin: 20px 0 40px 0;'/></a>
<?
		} else {
			?>
			<img src="/image2/etc/akbox_button.png" alt="참여하기"
					style="cursor: pointer;margin: 20px 0 40px 0;"
					onclick="Javascript:alert('로그인을 하셔야 참여가능합니다');window.location.href='/main/index.php?board=login';" />
<?
		}
	} else {
		?>
			...
			<? if(strcmp($_SESSION['temp_id'], '')){ ?>
				<a
									href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_02&idx=<?=$out_row['hero_idx']?>"><span
									class="bg1">신청자 확인</span></a>
			<? }else{ ?>
                
				<img src="/image2/etc/akbox_button.png" alt="미션참여하기"
									style="cursor: pointer;margin: 20px 0 40px 0;"
									onclick="Javascript:alert('로그인을 하셔야 참여가능합니다');window.location.href='/main/index.php?board=login';" />
			<? } ?>
...
if (! strcmp ( $setup_type, '2' )) {
	?>
					...
					<dl>
						<dt style="width: 100%; text-align: center">
							<a
								href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_03&idx=<?=$out_row['hero_idx']?>">
								<img src="/image2/etc/missionBtn03.jpg" alt="당첨자 확인" style="cursor: pointer;margin: 20px 0 40px 0;"/>
							</a> 
							<a
								href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_02&idx=<?=$out_row['hero_idx']?>">
...
<?
	if (! strcmp ( $data_count, '0' )) {
		?>
                <a
								href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_05&page=1&idx=<?=$_GET['idx']?>">
...
<?
		...
		if (! strcmp ( $new_count, '0' )) {
			?>
                <a
								href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_04&action=write&page=1&idx=<?=$_GET['idx']?>">
...
<?
	...
	?>
						...
						<div style="width: 100%; text-align: center">
							<a
								href="<?=PATH_HOME?>?board=<?=$_GET['board']?>&view=step_05&action=write&page=1&idx=<?=$_GET['idx']?>">
