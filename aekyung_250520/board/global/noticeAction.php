...
if($_GET["action"] == "write") {
	...
	$insert_sql .= " , hero_today, hero_use_yn, hero_notice, hero_temp ";
	if($userfile_count > 0) {
		$insert_sql .= $upload_column;
	}
	$insert_sql .= " ) VALUES ";
	$insert_sql .= " ('".$_SESSION["global_code"]."','".$_POST["hero_table"]."','".$_POST["board_code"]."','".$_POST["hero_title"]."' ,'".$_POST["hero_command"]."' ";
	$insert_sql .= " ,now(),'Y','".$_POST["hero_notice"]."','".$_POST["hero_temp"]."' ";
	if($userfile_count > 0) {
	...
	if($result) {
		message($msg);
		location(PATH_HOME."?board=".$_GET["board"]."&view=noticeList");
	}
	...
	if($result) {
		message($msg);
		location(PATH_HOME."?board=".$_GET["board"]."&view=noticeWrite&hero_idx=".$_POST["hero_idx"]);
	}
