...
if($_GET["action"] == "write") {
	...
	$insert_sql .= " , hero_country, hero_today, hero_use_yn, hero_notice, hero_temp, hero_pcMobile ";
	if($userfile_count > 0) {
		$insert_sql .= $upload_column;
	}
	$insert_sql .= " ) VALUES ";
	$insert_sql .= " ('".$hero_code."','".$_POST["hero_table"]."','".$_POST["board_code"]."','".$_POST["hero_title"]."' ,'".$_POST["hero_command"]."' ";
	$insert_sql .= " ,'".$_SESSION["global_country"]."' ,now(),'Y','".$_POST["hero_notice"]."','".$_POST["hero_temp"]."','".$_POST["hero_pcMobile"]."' ";
	if($userfile_count > 0) {
	...
	if($result) {
		message($msg);
		location(PATH_HOME."?board=".$_GET["board"]."&view=qnaList");
	}
} else if($_GET["action"] == "edit") {
	
	$where = "";
	if($_SESSION["global_admin_yn"] != "Y") {
		$where .= " AND hero_code = '".$_SESSION["global_code"]."' ";
	}
	...
	$edit_sql .= " WHERE hero_idx = '".$_POST["hero_idx"]."' ".$where;	
	...
	if($result) {
		message($msg);
		location(PATH_HOME."?board=".$_GET["board"]."&view=qnaWrite&hero_idx=".$_POST["hero_idx"]);
	}
	...
	$drop_sql = " UPDATE global_board SET hero_use_yn = 'N' WHERE hero_idx = '".$_POST["hero_idx"]."' AND board_code = '".$_POST["board_code"]."' ".$where;
