...
######################################################################################################################################################
if($action=="write"){
	$duplecateInsertCheck = "select hero_idx, hero_table from board where hero_code='".$_SESSION['temp_code']."' and hero_title='".$_POST['hero_title']."' and hero_table='".$_POST['hero_table']."' and left(hero_today,10)='".date("Y-m-d")."'";
	//echo $duplecateInsertCheck."<br/>";
	//exit;
	$duplecateInsertCheck_res = mysql_query($duplecateInsertCheck);
	if(!$duplecateInsertCheck_res){

		logging_error($_SESSION['temp_code'], $_GET['board']."-WRITE3_01 : ".$duplecateInsertCheck, date('Y-m-d H:i:s'));
		error_historyBack("");
...
$sql = 'select * from board where hero_idx=\''.$_GET['idx'].'\';';
sql($sql, 'on');
...
if(!strcmp($_GET['action'], 'update')){
    $msg .= '수정 되었습니다.';
    $sql = 'UPDATE '.$hero_table.' SET '.$sql_one_update.' WHERE hero_idx = \''.$_GET['hero_idx'].'\';';
    @mysql_query($sql);
    $get_herf = get('view||action||next_board||hero_idx','view=step_05');
}else if(!strcmp($_GET['action'], 'write')){
######################################################################################################################################################
    $msg .= '입력 되었습니다.';
//    $get_herf = get('view||action||idx||page','','');
    $get_herf = get('view||action||next_board','view=step_05','');
...
//        @unlink($filename);
    ...
    $point_rs = pointInsert($_GET['board'], "mission_write", $good_idx, 0, $_POST["hero_title"], "Y", $full_today );
    ...
    if(substr($point_rs,0,7)=='message'){
    	...
    	if(!$pf_rollback){
    		logging_error($_SESSION['temp_code'], $board."-STEP_01_05 : ".$rollback_query, $full_today);
    	}
    	error_historyBack("");
    	exit;
...
######################################################################################################################################################
    ...
    $temp_point = $right_list['hero_mission_join'];
    $temp_idx = $_GET['idx'];
...
######################################################################################################################################################
                if(!strcmp($level_up_ok, '0')){
                    ...
                    $sql_two_write = '\''.$good_idx.'\', \''.$temp_code.'\', \''.$_GET['board'].'\', \''.$_GET['view'].'\', \''.$temp_id.'\', \''.$temp_top_title.'\', \''.$temp_title.'\', \''.$_SESSION['temp_name'].'\', \''.$_SESSION['temp_nick'].'\', \''.$temp_point.'\', \''.Ymdhis.'\'';
...
######################################################################################################################################################
            ...
            $sql_one_write = 'hero_old_idx, hero_code, hero_table, hero_type, hero_id, hero_top_title, hero_title, hero_name, hero_nick, hero_point, hero_today';
            $sql_two_write = '\''.$good_idx.'\', \''.$temp_code.'\', \''.$_GET['board'].'\', \''.$_GET['view'].'\', \''.$temp_id.'\', \''.$temp_top_title.'\', \''.$temp_title.'(당일 포인트 초과)\', \''.$_SESSION['temp_name'].'\', \''.$_SESSION['temp_nick'].'\', \'0\', \''.Ymdhis.'\'';
//            $sql = 'INSERT INTO point ('.$sql_one_write.') VALUES ('.$sql_two_write.');';
//            mysql_query($sql);
        }
    } */
...
//                @unlink($check_file);
    ...
    sql($sql);
    $sql = 'DELETE FROM point WHERE hero_table=\''.$hero_table.'\' and hero_old_idx = \''.$_GET['idx'].'\';';
    @mysql_query($sql);
...
if(!$good_idx)			$href_location=$_GET['hero_idx'];
else					$href_location=$good_idx;
$action_href = PATH_HOME.'?'."board=group_04_09&page=".$_GET['page']."&view=view&idx=".$href_location."";
