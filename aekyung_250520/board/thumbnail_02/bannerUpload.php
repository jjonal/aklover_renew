...
if(is_array($_FILES)) {
	if(is_uploaded_file($_FILES['bannerFile']['tmp_name'])) {
		
	$sourcePath = $_FILES['bannerFile']['tmp_name'];
	$filePath = $_SERVER["DOCUMENT_ROOT"]."/image2/banner/";

	$arrExe = explode(".", $_FILES['bannerFile']['name']);
	$exetention = $arrExe[1];
	...
	$filenameTemp = $_FILES['bannerFile'][name];
	$num = 0;
	$exists = file_exists($filePath.$_FILES['bannerFile'][name]);
