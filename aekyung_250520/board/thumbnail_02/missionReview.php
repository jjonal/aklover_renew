...
$gisu_type_gubun = array(
		...
		,"group_04_27" => array("title"=>"Beauty Club 영상팀","column"=>"hero_movie_gisu","level"=>"9995")
		,"group_04_28" => array("title"=>"Life Club","column"=>"hero_life_gisu","level"=>"9994")
		,"group_04_31" => array("title"=>"Life Club 영상팀","column"=>"hero_movie_gisu","level"=>"9993")
				    	...
				    	<? if($board == "group_04_27") {?>
				    		...
				    		<select name="hero_movie_gisu" onChange="fnMovieGisu(this.value);">
			        			...
			        			<? foreach($res_gisu_list as $rs_gisu) { ?>
			        				<option value="<?=$rs_gisu[$gisu_column]?>" <?=$rs_gisu[$gisu_column]==$_GET["hero_gisu"] ? "selected":"";?>><?=$rs_gisu[$gisu_column]?>기</option>
			        			<? } ?>
			        		</select>
				    	
				    	<? } else { ?>
			        		<span class="titBox">· <?=$gisu_type_gubun[$board]["title"]?>  <? if($hero_gisu){?><?=$hero_gisu;?>기<? } ?></span>
			        		
			        		<select name="hero_gisu" onChange="fnGisu(this.value);">
			        			<option value=""><?=$gisu_type_gubun[$board]["title"]?> 기수 선택</option>
			        			<? foreach($res_gisu_list as $rs_gisu) { ?>
			        				<option value="<?=$rs_gisu[$gisu_column]?>" <?=$rs_gisu[$gisu_column]==$_GET["hero_gisu"] ? "selected":"";?>><?=$gisu_type_gubun[$board]["title"]?> <?=$rs_gisu[$gisu_column]?>기</option>
			        			<? } ?>
			        		</select>
		        		<? } ?>
		        	</div>
