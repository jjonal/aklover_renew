...
$temp_idx = "1288"; //20180831 해당 체험단  타이틀 명 및 항목 수정
...
if(!$temp_code){  
	error_location("로그인 후에 참여하실 수 있습니다.","/main/index.php?board=login");
	exit;
}
//중복참여 체크
...
$group_sql = " SELECT * FROM hero_group WHERE hero_board ='".$board."' "; //메뉴 정보
...
if(!strcmp($_GET['type'], 'edit' )) { //등록프로세스
	...
	if(!$sql_ch_res){
		logging_error($temp_nick, $board."-STEP_01_02", $full_today);
		error_historyBack("오류입니다.");
		exit;
	}
	
	$sql_ch_rs = mysql_fetch_assoc($sql_ch_res);
	if ($sql_ch_rs ['count_registered'] != 0) {
		echo "<script>alert('체험단에 이미 참여하셨습니다');location.href='" . PATH_HOME . "?board=" . $_GET ['board'] . "&view=view&idx=" . $_GET ['idx'] . "';</script>";
		exit();
	}
	...
	if($mission_review_idx > 0) {
		...
		//게시판명, 포인트타입, 게시글번호, 댓글번호, 제목, 포인트제한여부
...
<div class="contents" id="write_mode">
	<? if($idx == $temp_idx) {?>
		<p class="titleText fw500 main_c">이벤트 신청하기</p>
	<? } else { ?>
		<p class="titleText fw500 main_c">체험단 신청하기</p>
	<? } ?>
	<div class="bread">
		<ul class="f_sc">
	...
	<div class="step step01">
		...
		<?
		$number=1;
		if($mission_board_type == false && $out_row["hero_question_url_yn"] != "N"){ //타입(소문내기) 아닌경우
		?>
		<dl class="sns_input_bx">  
			...
			<dd>
				...
				<ul>
					<li>
						<span class="fz19 fw600 li_tit">SNS 정보를 입력해주세요.</span>
						...
						<? } else if($out_row['hero_question_url_check'] == "6") {?>
							<span class="txt_emphasis_12">* 네이버 블로그/인스타그램/영상 채널(유튜브, 네이버TV, 틱톡, 기타 등)<br /> URL 중 한개의 URL은 필수로 입력하셔야 합니다.</span>
						<? } ?>
					</li>
				</ul>
				<ul class="sns_input">
					...
					foreach($hero_question_url_list as $key => $value) {
						...
						case "블로그":
							...
							<li>
								...
								<div>&nbsp;&nbsp;https://blog.naver.com/<input type="text" maxlength="25" id="hero_blog_<?=$i?>" name="<?=$blog_id?>" class="hero_blog" value="<?=str_replace("https://blog.naver.com/", "", $member_rs["{$blog_id}"]);?>" placeholder="네이버 ID 또는 블로그 ID를 입력해주세요."></div>
							</li>
							<? 
						break;
						...
						case "인스타그램":
							...
							<li>
								<span><?=$value=="블로그" ? "네이버 블로그":$value?> URL :</span>
								<div>&nbsp;&nbsp;https://www.instagram.com/<input type="text" maxlength="25" id="hero_blog_<?=$i?>" name="<?=$blog_id?>" class="hero_blog" value="<?=str_replace("https://www.instagram.com/", "", $member_rs["{$blog_id}"]);?>" placeholder="인스타그램 ID를 입력해주세요."></div>
							</li>
							<? 
						break;
						...
						case "페이스북":
							...
							<li>
								<span><?=$value=="블로그" ? "네이버 블로그":$value?> URL :</span>
								<div><input type='text' id="hero_blog_<?=$i?>" name="<?=$blog_id?>" class="hero_blog" value="<?=url($member_rs["{$blog_id}"]);?>" placeholder="SNS URL을 입력해주세요.(http:// 또는 https://필수 입력)"></div>
							</li>
							<? 
						break;
				...
				<? } else { ?>
					<ul>
						<li class="fz26 fw500">※ 아래 사항을 입력해 주세요.</li>
					</ul>
				<? } ?>         
			</dd>
		</dl>		
		<div class="btn_bx f_c">
			<div class="btn_submit btn_black" onClick="stepOneChk(document.form_next)">다음으로</div>
		</div>
		<? } ?>
		<!------------------- 소문내기 폼 ------------------------------>
		<? if($mission_board_type){?>
			...
			<div class="sns_input_bx rumor_box">
				<ul class="sns_input">
					<li>
						...
						<div><input type="text" name="hero_title" id="hero_title" title="제목" style="width:480px"/></div>
					</li>
					<li>
						<span>작성자</span>
					...
					<li>
						<span class="required">대표이미지</span>
						<div>
					...
					<!-- 250224 선택 영역 추가 -->
                    <li class="sns_tit">
                        <p class="fz19 fw600 li_tit">SNS 정보를 입력해주세요.</p>
                        <span class="txt_emphasis_12">
                            <?php
                                $message = '';

                                if ($out_row['hero_question_url_check'] == '1') { // 네이버블로그
                                    $message = '* 네이버블로그 URL은 필수로 입력하셔야 합니다.';
                                } elseif ($out_row['hero_question_url_check'] == '2') {  // 인스타그램
                                    $message .= '* 인스타그램 URL은 필수로 입력하셔야 합니다.';
                                } /*elseif ($out_row['hero_question_url_check'] == '5') { // 후기(영상)
                                    $message .= '* 영상채널(유튜브, 네이버TV, 틱톡, 기타 등) URL은 필수로 입력하셔야 합니다.';
                                }*/ elseif ($out_row['hero_question_url_check'] == '3') {  // 네이버 블로그 or 인스타그램
                                    $message = '* 네이버블로그/인스타그램 URL중 한개의 URL은 필수로 입력하셔야 합니다.';
                                } elseif ($out_row['hero_question_url_check'] == '4') {  // 네이버 블로그 and 인스타그램
                                    $message = '* 네이버블로그/인스타그램 URL은 필수로 입력하셔야 합니다.';
                                } /*elseif ($out_row['hero_question_url_check'] == '6') {  // 네이버 블로그 or 인스타그램 or 후기(영상)
                                    $message = '* 네이버블로그/인스타그램/영상채널(유튜브, 네이버TV, 틱톡, 기타 등) URL중 한개의 URL은 필수로 입력하셔야 합니다.';
                                }*/
					...
					<li>
                        ...
                        <!--<span class="<?php /*= (strpos($out_row["hero_question_url_list"], "블로그") !== false) ? 'required' : '' */?>">
							네이버 블로그
						</span>-->
                        <span>
							네이버 블로그
						...
						<div>
							<input type="text" name="naver_url" placeholder="반드시 포스팅 URL을 입력해주세요.(http:// 또는 https://필수 입력)" class="inputUrl2"/>
							...
							<dl class="urlAgreeBox urlAgreeBoxType2">
								...
								<dd><div class="input_radio"><input type="radio" name="naver_member_check" id="naver_member_check_Y" value="Y"/><label for="naver_member_check_Y">예</label></div>
									<div class="input_radio"><input type="radio" name="naver_member_check" id="naver_member_check_N" value="N"/><label for="naver_member_check_N">아니오</label></div>
								</dd>
							</dl>
					...
					<li>
						<!--<span class="<?php /*= (strpos($out_row["hero_question_url_list"], "인스타그램") !== false) ? 'required' : '' */?>">
							인스타그램
						</span>-->
                        <span>
							인스타그램
						...
						<div>
							<input type="text" name="insta_url" placeholder="반드시 포스팅 URL을 입력해주세요.(http:// 또는 https://필수 입력)" class="inputUrl2"/>
							...
							<dl class="urlAgreeBox urlAgreeBoxType2">
								<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
								<dd><div class="input_radio"><input type="radio" name="insta_member_check" id="insta_member_check_Y" value="Y"/><label for="insta_member_check_Y">예</label></div>
									<div class="input_radio"><input type="radio" name="insta_member_check" id="insta_member_check_N" value="N"/><label for="insta_member_check_N">아니오</label></div>
								</dd>
							</dl>
							<p class="txt_agree_info mgb5">
								※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK Lover 활동에 불이익이 있을 수 있습니다.
					...
					<? if($_GET['board'] == 'group_04_27'){ //뮤자인 리뉴얼이후 사용X ?>
						<li>
							<span>후기(영상)</span>
							<div>
								<div class="ui_urlBox">
									<div class="ui_url">
										...
										<dl class="urlAgreeBox urlAgreeBoxType2">
											<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
											<dd><div class="input_radio"><input type="radio" name="movie_member_check1" id="movie_member_check1_Y" value="Y"/><label for="movie_member_check1_Y">예</label></div>
												<div class="input_radio"><input type="radio" name="movie_member_check1" id="movie_member_check2_N" value="N"/><label for="movie_member_check2_N">아니오</label></div>
											</dd>
										</dl>
										<p class="txt_agree_info mgb5">
											※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK Lover 활동에 불이익이 있을 수 있습니다.
						...
						<div>
							<div class="ui_urlBox">
								<div class="ui_url">
									...
									<dl class="urlAgreeBox urlAgreeBoxType2">
										<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
										<dd><div class="input_radio"><input type="radio" name="cafe_member_check1" id="cafe_member_check1_Y" value="Y"/><label for="cafe_member_check1_Y">예</label></div>
											<div class="input_radio"><input type="radio" name="cafe_member_check1" id="cafe_member_check1_N" value="N"/><label for="cafe_member_check1_N">아니오</label></div>
										</dd>
									</dl>
									<p class="txt_agree_info mgb5">
										※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK Lover 활동에 불이익이 있을 수 있습니다.
						...
						<div>
							<div class="ui_urlBox">
								<div class="ui_url">
									...
									<dl class="urlAgreeBox urlAgreeBoxType2">
										<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
										<dd><div class="input_radio"><input type="radio" name="etc_member_check1" id="etc_member_check1_Y" value="Y"/><label for="etc_member_check1_Y">예</label></div>
		...
		<script type="text/javascript">
			...
			<div class="btn_bx f_c">
				<div class="btn_submit btn_black" onClick="stepAlertChk(document.form_next)">다음으로</div>
			</div>
		<? } ?>
		<!-------------------------- 소문내기 폼 끝 --------------------------->
	</div>		
	...
	<div class="step step02" style="">		
		...
		<ul class="sns_input_bx">
			<li><span class="fz19 fw600 li_tit">필수 확인 사항을 체크해주세요.</span></li>
		</ul>
			...
			<li>
				<div class="agree">
					...
					<ul>
						...
						<li class="agreechk">
							<div>
								<p class="fz15 fw600">위 내용에 동의하십니까?</p>
								<p class="fz14 fw500">
							...
							<div class="input_chk"><input type="checkbox" name="hero_agree" id="hero_agree" value='Y'><label for="hero_agree" class="input_chk_label">확인</label></div>
						</li>
					</ul>
				</div>
			</li>
		...
		<div class="btn_bx f_c">
			<? if( $mission_board_type || ($mission_board_type == false && $out_row["hero_question_url_yn"] != "N") ){?>
			<div class="btn_submit btn_gray btn_prv">이전으로</div>
			<? } ?>
			<div class="btn_submit btn_black" onClick="stepTwoChk(document.form_next)">다음으로</div>
		</div>
	</div>		
	<div class="step step03 mu_support" style="">		
		<p class="fz15 fw600 main_c step_count">step.</p>		
		<ul class="sns_input_bx">
			<li><span class="fz19 fw600 li_tit">필수 항목을 입력해주세요.</span></li>
		</ul>						
			...
			<dd>
				<ul>
					<!-- <? if($idx == $temp_idx) {
						...
						<li class="c_orange">(필수) 이벤트 상품을 배송 받을 주소를 입력해주세요.</li>
					<? } else { ?>
						<li class="c_orange">(필수) 포인트 체험단 혜택을 수령하실 휴대폰 번호를 입력해주세요.</li>
					<? } ?>
					<? } else { 
						if($out_row['hero_type'] != 8) {?>
						<li class="c_orange">(필수) 체험단 상품을 배송 받을 주소를 입력해주세요.</li>
					<? } else { ?>
						<li class="c_orange">(필수) 포인트 체험단 혜택을 수령하실 휴대폰 번호를 입력해주세요.</li>
					<? } ?>
					<? } ?> -->
					<li><label for="hero_new_name">받으시는분</label> 
						<input type="text" name="hero_new_name" id="hero_new_name" value="<?=$member_rs['hero_name']?>">
					</li>
					<? if($out_row['hero_type'] != 8) { ?>
						<li>
							<label for="hero_address_01">주소</label>
							<input type="text" name="hero_address_01" id="hero_address_01" value="<?=$member_rs['hero_address_01']?>"  style="width:380px;" onclick="javascript:btnAddressGet()" readonly /> <a href="javascript:btnAddressGet()" class="btn_post">우편번호</a><br />
					...
					<?
						...
						$next = str_ireplace ( '/', '', $next );
					?>
					<li><label for="hero_hp_01">연락처</label> 
						<div class="f_b phone">
					...
					<? if($out_row['hero_type'] == 8) { ?>
						<li style="margin-bottom: 0;">
							<label>포인트 체험단 진행 과정 확인</label> 
							<span class="fz14 op05">포인트 체험단은 사전 안내된 구매 URL을 통해 구매 기간 내 직접 당첨된 제품을 구매하시어 필수 미션을 진행해주셔야 되며, 필수 미션 진행자를 대상으로 구매 비용 환급이 아닌 안내 된 혜택으로 지급됩니다. (진행 체험단 별 혜택 상이함)</span>
							<div class="c_orange f_b" style="margin-top: 3rem;">
								<p class="fz16 fw500">위 내용을 확인하셨습니까?</p>
								<div class="input_chk confirm_chk"><input type="checkbox" name="hero_confirm" id="hero_confirm" value='Y'><label for="hero_confirm"  class="input_chk_label">확인</label><br/>
							</div>
						</li>
					<? } else {?>							
						<li class="f_cs" style="margin-bottom: 0;">
							<div class="input_chk"><input type="checkbox" name="hero_address_save" id="hero_address_save" value="1"/><label for="hero_address_save" class="input_chk_label">기본 주소로 저장</label></div>
							<span class="fz14  op05" style="margin-left: 1rem;">* 선택 시 회원정보 휴대폰번호, 주소가 변경됩니다.</span>
						</li>
					<? } ?>						
				</ul>
			</dd>
				...
				<ul>
					<li class="point_radio">
						<div class="f_cs"> 
							...
							<div class="input_radio"><input type="radio" name="delivery_point_yn" id="delivery_point_yn1" value='Y' style="width: 20px;"><label for="delivery_point_yn1" style="width: 20px;">예</label></div>
							<div class="input_radio"><input type="radio" name="delivery_point_yn" id="delivery_point_yn2" value='N' style="width: 20px;" checked><label for="delivery_point_yn2" style="width: 50px;">아니오</label></div>
						</div>
						<span class="fz14 500">* <?=$_DELIVERY_POINT?>포인트 차감하지 않을 시 체험단 제품은 착불로 배송됩니다.</span>
					</li>
		...
		<dl>
			<dt><span class="question_num">슈퍼패스 사용 여부</dt>
			<dd>
				<ul class="superpassBox">
					<? if($member_rs["superpass_count"]==0){ ?>
						<li class="fz15 bold">사용할 수 있는 슈퍼패스가 없습니다.</li>						
					<? } else if(countSuperpass($out_row['hero_select_count'])<=$out_row['enrolled_superpass']){ ?>
						<li class="fz15 bold">본 체험단의 슈퍼패스는 선착순으로 마감되었습니다.</li>
		...
		<div class="btn_bx f_c">
			<div class="btn_submit btn_gray btn_prv">이전으로</div>
			<input type="button" class="btn_submit btn_color" onClick="go_submit(this.form)" value="서포터즈 신청 하기"/>
		</div>
	</div>
	</div>
</form>
...
<script type="text/javascript">
	...
	*/
	...
	function stepAlertChk(form){
        ...
        if(hero_question_url_check == "1"){ //URL필수값 네이버
            if(!$("input[name='naver_url']").val()) { //네이버
                alert("네이버블로그 URL은 필수로 입력하셔야 합니다.");
                $("input[name='naver_url']").focus();
                return;
            }
        } else if (hero_question_url_check == "2"){ //URL필수값 인스타
            if(!$("input[name='insta_url']").val()) { //인스타
                alert("인스타그램 URL은 필수로 입력하셔야 합니다.");
                $("input[name='insta_url']").focus();
                return;
            }
        } else if(hero_question_url_check == "3" || hero_question_url_check == "6"){ //URL필수값 네이버 or 인스타
            if(!$("input[name='naver_url']").val() && !$("input[name='insta_url']").val()){ //네이버 or 인스타
                alert("네이버블로그/인스타그램 URL중 한개의 URL은 필수로 입력하셔야 합니다.");
                $("input[name='naver_url']").focus();
                return;
            }
        } else if(hero_question_url_check == "4"){ //URL필수값 네이버 and 인스타
            if(!$("input[name='naver_url']").val()) { //네이버블로그
                alert("네이버블로그 URL은 필수로 입력하셔야 합니다.");
                $("input[name='naver_url']").focus();
                return;
            }

            if(!$("input[name='insta_url']").val()) { //인스타그램
                alert("인스타그램 URL은 필수로 입력하셔야 합니다.");
                $("input[name='insta_url']").focus();
                return;
            }
        ...
        if(!cafeUrlCheck) return; //카페 url형식 체크
        if(!cafeMemberCheck) return; //카페 공정거래위원회 문구 작성 체크여부
        ...
        if(!etcUrlCheck) return; //기타 url형식 체크
        if(!etcMemberCheck) return; //기타 공정거래위원회 문구 작성 체크여부
	...
	function stepTwoChk(form){
		...
		if(!form.hero_agree.checked){
			alert("콘텐츠 활용에 동의해야 체험단 신청이 가능합니다");
	...
	var hero_blog = []; //hero_blog.length; 20140410 선택할 수 있는 블로그의 갯수 
	function ch_disabled(value_item){ //20140410 저장할 블로그 선택 변경시 disabled 변경
			...
			if(confirm("슈퍼패스를 사용하시겠습니까?")){
				    ...
				    $.ajax({      
				        ...
				        success:function(args){   
				            if(args==0){
								if(confirm("본 체험단의 슈퍼패스는 이미 선착순으로 마감되었습니다. 슈퍼패스 없이 체험단에 참여하시겠습니까?")){
					        ...
					        }else if(args==9)	alert("시스템 에러입니다2. 다시 시도해 주세요.");
				        }   
