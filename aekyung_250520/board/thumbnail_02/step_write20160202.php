...
function add_question_area(){
	...
	var add_html = "<dd alt='"+last_count+"'><textarea class='textarea hero_ask' style='width: 515px; height: 50px;float:left;' ></textarea><div style='width: 33px;float: left;'><p class='step_write_plus_btn' onclick='add_question_area();'>+</p><p class='step_write_plus_btn' onclick='minus_question_area("+last_count+");' style='margin-top: 3px;'>-</p></div></dd>";
...
<style>
	...
	<div class="contents">
		...
		<form name="frm" method="post"	action="<?=MAIN_HOME;?>?board=<?=$_GET['board'];?>&view=action_write&action=<?=$_GET['action']?>&idx=<?=$_GET['idx'];?>&page=<?=$_GET['page'];?>">
			<div class="spm_img">
			...
			<div class="spm_txt">
				<dl>
					...
					<dd>
						...
						<input type="radio" name="hero_type" value="1" <?echo ($view_row['hero_type']==1)? "checked='checked'" : "" ;?>/> 이벤트
					</dd>
				</dl>
				<dl>
					<dt>
						<span class="bg1">수퍼패스 여부</span>
					</dt>
					<dd>
						<input type="radio" name="hero_superpass" value="Y" <?echo ($view_row['hero_superpass']=='Y')? "checked='checked'" : "" ;?>/> 사용
						<input type="radio" name="hero_superpass" value="N" <?echo ($view_row['hero_superpass']=='N')? "checked='checked'" : "" ;?>/> 미사용
					</dd>
				</dl>
				<dl>
					<dt>
				...
				<dl>
					...
					<dd>
						...
						onkeydown="ch_count_text(54,'hero_required');" onblur="ch_count_text(54,'hero_required');"
				...
				<dl>
					...
					<?php }else{ 
						$mission_text = "- 리뷰 등록 시 스폰서 배너를 반드시 부착해주세요.<br/>- 리뷰 제품 배송료(2500원)은 본인 부담입니다.<br/>- 브랜드 요청에 따라서 인원 및 경품이 변경될 수도 있습니다.<br/>- 핸드폰보다는 카메라로 찍은 사진을 활용해주세요.<br/><br/><span style='color:red;'>* 기간 내 리뷰 미 등록시 페널티가 주어지게 되며, 차후 미션 참여에 불이익을 받을 수 있습니다.</span>";?>
...
<script>
$(function(){
	$("#sdate5").AnyTime_picker( {
    format: "%Y-%m-%d %H:%i:00"
 	});
	$("#edate5").AnyTime_picker( {
    format: "%Y-%m-%d %H:%i:00"
 	}); 
});
 </script>									
				...
				<dl>
						<span style="color:#B19D75">* 후기 등록 기간이 필요 없을 경우 - 리뷰어 발표 기간 마지막 날과 후기 등록 기간 마지막날을 같은 날로 설정해주세요.</span>
				</dl>
				<dl>
					<dt>
						<span class="bg1">후기 베스트 발표</span>
					</dt>
					<dd>
				...
				<dl>
					...
					<dd>
						<textarea name="hero_banner" id="hero_banner" class="textarea" style="width: 550px; height: 150px;"><?=$view_row['hero_banner'];?></textarea>
					</dd>
				</dl>
				...
				<dl id="step_write_question_area" >
					<dt>
						...
						?>
								<dd alt="<?=$ask_i?>">
									<textarea class="textarea hero_ask" style="width: 510px; height: 50px;float: left;"><?=$value;?></textarea>
