...
function fnXssClean($array_param) {
	foreach($array_param as $key => $val) {
		if(is_array($val)) {
			...
			if($key == "hero_nick") {
				...
				$param[$key] = htmlspecialchars($val);
			}
		}
	}
	return $param;
...
$_POST = fnXssClean($_POST);
$_GET = fnXssClean($_GET);
$_REQUEST = fnXssClean($_REQUEST);
####################################################################################################################################################
...
define('ADMIN_DEFAULT',                                             '/loaksecure21',TRUE);                                                                 //기본폴더/설치경로  예) admin 기본폴더일경우 '/admin'표시됨
...
ini_set('session.gc_maxlifetime', '1800'); //세션의 시간설정 3600은 60초*60분이다
ini_set("upload_max_filesize","100M");
ini_set('max_upload_size', '100M');
ini_set('memory_limit', '100M');
ini_set("post_max_size","100M");
session_set_cookie_params(0,  "/");
ini_set("session.cookie_domain", ".aklover.co.kr"); //배포용
//ini_set("session.cookie_domain", "58.229.163.192"); //테스트용
set_time_limit(0);
@session_start();
...
####################################################################################################################################################
//도메인 최상의 경로
define('DOMAIN',                                                    'https://www.aklover.co.kr',TRUE); //배포용                        //http://a.com
//define('DOMAIN',                                                    'http://aklover-test.musign.kr',TRUE); //뮤자인 테스트 링크 musign
//define('DOMAIN',                                                    '58.229.163.192',TRUE); //테스트용                      //http://a.com
...
define('PATH_HOME_HTTPS',											'https://www.aklover.co.kr'.PATH_DEFAULT.SLASH.PAGE_DEFAULT,TRUE);  //배포용 160616 추가
//define('PATH_HOME_HTTPS',											'http://aklover-test.musign.kr'.PATH_DEFAULT.SLASH.PAGE_DEFAULT,TRUE);  //뮤자인 테스트 musign
//https://a.com/현재폴더/index.php
//define('PATH_HOME_HTTPS',											'58.229.163.192'.PATH_DEFAULT.SLASH.PAGE_DEFAULT,TRUE);  //테스트용 160616 추가 //https://a.com/현재폴더/index.php
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
$_DELIVERY_POINT = 1000; //24.07.08 musign ak측 요청으로 500에서 1000으로 변경.
...
//$_FAQ_CATEGORY = array("전체", "홈페이지 이용", "체험단 신청", "체험단 당첨", "체험단 진행", "포인트 및 페널티", "회원정보", "기타");
$_FAQ_CATEGORY = array("전체", "홈페이지", "체험단 신청", "체험단 진행", "포인트", "개인정보", "슈퍼패스", "패널티", "이벤트", "기타");
$_NOTICE_CATEGORY = array("전체", "공지사항", "홈페이지", "체험단");
$_MAIN_COLOR = array("#ff0030","#2d0aff");

//TODO 테스트때문에 추가
/*
if($_SESSION["aklover"] != "yes") {
