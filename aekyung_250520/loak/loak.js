...
function setConfig() {
    var config = {
        ...
        tabIndex            : 0,
        thumbnailWidth      : 120,
        oncontextmenu       : true, //191031 특정 게시판 우클릭 금지 변수 추가
...
cheditor.prototype = {
    ...
    setDocumentBodyProp : function () {
        ...
        if(!this.config.oncontextmenu) {
        	this.doc.body.setAttribute('oncontextmenu', 'return false');
        	//this.doc.body.setAttribute('ondragstart', 'return false');
        	//this.doc.body.setAttribute('onselectstart', 'return false');
        }
    },
    ...
    doInsertImage : function (imgs, para, insertSpace) {
        ...
        for (i in imgs) {
			...
			//###################### 추가시작	######################//
            if(this.inputForm == "editor") {
				var dns = window.location.href;
				var arrDns = dns.split("//");
				var str = arrDns[0]+arrDns[1].substring(0, arrDns[1].indexOf("/"));			//도메인 주소
				var cnt = parseInt(document.getElementById('thumbCount').value);			//이미지 개수 초기값
		
				var imgThumb = new Image();
				imgThumb.setAttribute('src', imgAttr.fileUrl);
				imgThumb.setAttribute('alt', imgAttr.alt ? imgAttr.alt : imgAttr.origName);
				imgThumb.style.border = 'none';
		
				if(parseInt(imgAttr.width) >= parseInt(imgAttr.height)){
					imgThumb.setAttribute('width', 124);
					if(GB.browser.msie) imgThumb.setAttribute('height', 94);
				}else{
					imgThumb.setAttribute('height', 94);
					if(GB.browser.msie) imgThumb.setAttribute('width', 124);
				}
		
				var imgBox = document.createElement('DIV');
				imgBox.id = 'imgBox' + cnt;
				...
				theImg.appendChild(imgThumb);
				document.getElementById('thumbnailView').appendChild(imgBox);
		
				document.getElementById('imgBox' + cnt).onclick = function(){
					document.getElementById('hero_thumb').value = $(this).find("img").attr("src");
					$('#thumbnailView > div').each(function(){
						if($(this).attr("class")=="imageBoxHighlighted") $(this).removeClass("imageBoxHighlighted").addClass("imageBox");
					});
					$(this).removeClass("imageBox").addClass("imageBoxHighlighted");
				};
				
				if(cnt==0){
					document.getElementById('hero_thumb').value = imgAttr.fileUrl;
					$('#imgBox' + cnt).removeClass("imageBox").addClass("imageBoxHighlighted");
				}
				cnt = cnt + 1;
				document.getElementById('thumbCount').value = cnt;
            }
			//###################### 추가끝		######################//
	...
	resizeImage : function (img) {
		console.log(img);
		if (this.resizeImageComplete(img)) {
			console.log('2');
