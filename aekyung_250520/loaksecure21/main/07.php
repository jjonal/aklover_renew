...
if(strcmp($_POST['kewyword'], '') || strcmp($_GET['kewyword'], '')){
	...
	$sql = "select count(*) as count from board A where A.hero_table in ('group_04_05', 'group_04_06', 'group_04_07', 'group_04_08', 'group_04_09', 'group_04_10') and A.hero_board_three='1' ".$search." order by A.hero_today desc, A.hero_idx desc;";
	//echo $sql;
	sql($sql);
					...
					<div class="searchbox" style="margin-top:20px;margin-bottom:20px;margin-left:20%">
                            ...
                            <form name="frm" action="<?=PATH_HOME.'?'.get('page');?>" method="POST" >
                                <select name="select" id="" style='width:80px;'>
									<option value="hero_nick"<?if(!strcmp($_REQUEST['select'], 'hero_title')){echo ' selected';}else{echo '';}?>>닉네임</option>
...
<table class='t_list'>
	...
	<tbody>
		...
		<tr>
			...
			<td><a href="<?=ADMIN_DEFAULT?>/index.php?board=main&idx=60&idx2=<?=$sql_group['hero_idx']?>" class="btn_blue2">사용</a></td>
		</tr>
		<?
					$i--;
					}
...
<div style="float:left;margin:12% 2% 0% 2%;"><img src='/image2/etc/black.png' width='30px;'></div>
...
if($_GET['use'] && !$_POST['kewyword']){
	...
	$idx = $_GET['id'];
		if($use==1){
			...
			$sql = "select count(*) from board where hero_table in ('group_04_05', 'group_04_06', 'group_04_07', 'group_04_08', 'group_04_09', 'group_04_10') and hero_notice_use=1;";
			//echo $sql;
			sql($sql);
			...
			//echo $count;
			if($count>=4){
				echo "<script>alert('러버스타 리뷰는 4개까지 등록할 수 있습니다.')</script>";
			}else{
...
<div style="float:left;width:47%">
	<div class='mainmain'><p>등록된 러버스타 리뷰</p></div>
	...
	<table class='t_list'>
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<a href="<?=ADMIN_DEFAULT?>/index.php?board=main&idx=60&use=<?=$sql_group['hero_notice_use']?>&id=<?=$sql_group['hero_idx']?>" class="btn_blue2">사용</a>
				</td>
				<td>
					<a href="<?=ADMIN_DEFAULT?>/index.php?board=main&idx=60&use=<?=$sql_group['hero_notice_use']?>&id=<?=$sql_group['hero_idx']?>" class="btn_blue2">취소</a>
				</td>
				<td><a href="<?=ADMIN_DEFAULT?>/index.php?board=main&idx=60&remove=<?=$sql_group['hero_idx']?>" class="btn_blue2">제거</a></td>
			</tr>
			<?
				$i++;
				}
