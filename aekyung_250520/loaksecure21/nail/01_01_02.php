...
######################################################################################################################################################
$sql = '
    select * from mission_review where
'.$search.' hero_old_idx=\''.$_GET['hero_idx'].'\'';
sql($sql);
...
?>
                        <center>
                            <a href="<?=PATH_HOME.'?'.get("view||table_type||table_name","view=01_01_01")?>" class="btn_blue2">이전목록</a><br><br>
...
<?
                        ...
                        if(!strcmp($_REQUEST['table_type'], 'hero_address')){
                            $view_list_end = nl2br($list['hero_address_01'])." ".nl2br($list['hero_address_02'])." ".nl2br($list['hero_address_03']);
                        }else if(!strcmp($_REQUEST['table_type'], 'hero_hp')){
                            $view_list_end = nl2br($list['hero_hp_01'])."-".nl2br($list['hero_hp_02'])."-".nl2br($list['hero_hp_03']);
                        }else{
                            $search_table_type = $_REQUEST['table_type'];
                            $view_list_end = nl2br($list[$search_table_type]);
                        }
                        ?>
