...
if($_GET["board_write"]) {
	if($_GET["board_write"] == "Y") {
		$search .= " AND  r.url_cnt > 0 ";
	} else if($_GET["board_write"] == "N") {
		$search .= " AND  (r.url_cnt = 0 || r.url_cnt is null) ";
	} else if($_GET["board_write"] == "W") {
		$search .= " AND  r.url_cnt > 0 AND r.hero_board_three = '1' ";
...
$sql .= " SELECT r.hero_idx, r.hero_new_name ";
$sql .= " , m.hero_name, m.hero_id, m.hero_nick, b.hero_board_three ";
...
$sql .= " WHERE m.hero_use=0 AND r.hero_old_idx='".$_GET['hero_idx']."' AND r.lot_01 = '1' GROUP BY r.hero_idx ";
$sql .= " ) r WHERE 1=1 ".$search;
...
$sql .= " , b.hero_idx as board_hero_idx , b.hero_board_three ";
$sql .= " , sum(if(u.url is not null || u.url != 0,'1','0')) as url_cnt ";
...
$thanksPoint = 500;
$unReviewPoint = 1000;
if($mission_rs["hero_type"] == 2 || $mission_rs["hero_type"] == 3 || $mission_rs["hero_type"] == 5) {
    $greatPoint = 0;
    $thanksPoint = 0;
}
?>
...
<div class="view_title_box">
	...
	<p><label>우수후기 선정</label> : <?=$greatPoint?>p, <label>감사포인트</label> : <?=$thanksPoint?>p, <label>후기 미등록</label> : -<?=$unReviewPoint?>p</p>
</div>
...
<div class="btnGroupFunction">
	<div class="leftWrap">
		<div class="mission">
			...
			<input type="radio" name="functionType" id="functionType_3" value="unReview" class="ml10"><label for="functionType_3">후기 미등록</label>
...
<thead>
	<tr>
		...
		<th rowspan="2">닉네임</th>
		<th rowspan="2">이름</th>
		<th rowspan="2">받는이름</th>
		<th rowspan="2">받는 휴대폰</th>
		<th rowspan="2">나이</th>
...
$(document).ready(function(){
	...
	fnExec = function() {
		...
		} else if($("input:radio[name='functionType']:checked").val() == "unReview") {
			confirm_txt = "후기 미등록 실행하시겠습니까?";
			$("#listForm input[name='mode']").val("unReview");
			$("#listForm input[name='point']").val('<?=$unReviewPoint?>');
		}

		if(confirm(confirm_txt)) {
			$.ajax({
				url:"/loaksecure21/nail/01_02_action.php"
				,type:"POST"
	...
	fnCancel = function() {
		...
		if($("input:radio[name='functionType']:checked").val() == "best") {
			...
			confirm_txt = "후기 미등록을 취소 실행하시겠습니까?";
			$("#listForm input[name='mode']").val("unReviewCancel");
			$("#listForm input[name='point']").val('<?=$unReviewPoint?>');
		}
	...
	fnExcel = function() {
		$("#searchForm").attr("action","/loaksecure21/nail/01_02_excel.php").submit();
	}

	fnExcelReview = function() {
		$("#searchForm").attr("action","/loaksecure21/nail/01_02_review_excel.php").submit();
	}
})
</script>
