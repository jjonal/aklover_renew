...
$memo_date =  "(".substr(date("Y"),2,2).".".date("m").")";
...
if($mode == "best") {	
	...
	for($i=0; $i<count($_POST["hero_idx"]); $i++) {
		$result = false;
		$board_sql  = " SELECT b.hero_code, b.hero_idx, b.hero_name, b.hero_nick, b.hero_board_three ";
		$board_sql .= "	, r.hero_id, r.hero_old_idx ";
		$board_sql .= "	, (SELECT hero_title FROM mission WHERE hero_idx = r.hero_old_idx) as mission_title ";
		$board_sql .= "	FROM mission_review r ";
		$board_sql .= " INNER JOIN board b ON r.hero_old_idx = b.hero_01 AND r.hero_code = b.hero_code ";
		$board_sql .= " INNER JOIN mission_url u ON b.hero_idx = u.board_hero_idx ";
		$board_sql .= " WHERE  r.hero_idx = '".$_POST["hero_idx"][$i]."' "; //후기가 등록이 되어 있어야 함
		...
		if($board_rs["hero_idx"] && ($board_rs["hero_board_three"] == "" || $board_rs["hero_board_three"] == "0")) {
			...
			//포인트 지급
			if($point > 0) {
			 adminPoint($board_rs["hero_code"], '', 'adminGreatPost', '', $board_rs["hero_old_idx"], '', $board_rs["hero_id"], $board_rs["mission_title"].' 우수후기 혜택', $board_rs["hero_name"], $board_rs["hero_nick"], $point, "N", '');
			}
		}
		if($result) {
			$success_num++;
	...
	for($i=0; $i<count($_POST["hero_idx"]); $i++) {
		...
		if($board_rs["hero_idx"] && $board_rs["hero_board_three"] == "1") {
			...
			//포인트 차감
			adminPoint($board_rs["hero_code"], '', 'adminGreatPostCancel', '', $board_rs["hero_old_idx"], '', $board_rs["hero_id"], $board_rs["mission_title"].' 우수후기 혜택 취소', $board_rs["hero_name"], $board_rs["hero_nick"], $point, "N", 'minus');
		}
	...
	for($i=0; $i<count($_POST["hero_idx"]); $i++) {
		...
		$sql .= " , (SELECT hero_title FROM mission WHERE hero_idx = r.hero_old_idx) as mission_title ";
		$sql .= " FROM mission_review r ";
		$sql .= " INNER JOIN board b ON r.hero_old_idx = b.hero_01 AND r.hero_code = b.hero_code ";
		$sql .= " INNER JOIN mission_url u ON b.hero_idx = u.board_hero_idx ";
		$sql .= " INNER JOIN member m ON r.hero_code = m.hero_code ";
		$sql .= " WHERE r.hero_idx = '".$_POST["hero_idx"][$i]."' ";
...
} else if($mode == "unReview") {
	$point = $_POST["point"];
	
	for($i=0; $i<count($_POST["hero_idx"]); $i++) {
		$result = false;
		$sql  = " SELECT m.hero_id, m.hero_nick, m.hero_name, m.hero_code ";
		$sql .= " , r.hero_old_idx, s.hero_title as mission_title FROM mission_review r ";
		$sql .= " INNER JOIN member m ON r.hero_code = m.hero_code ";
		$sql .= " INNER JOIN mission s ON r.hero_old_idx = s.hero_idx ";
		$sql .= " WHERE r.hero_idx = '".$_POST["hero_idx"][$i]."' AND r.lot_01 = '1' ";
		$res = sql($sql,"on");
		$rs = mysql_fetch_assoc($res);
		
		$sql_penalty = " SELECT count(*) cnt FROM member_penalty WHERE hero_code = '".$rs["hero_code"]."' AND mission_idx = '".$rs["hero_old_idx"]."' AND type='3' AND hero_use_yn = 'Y' ";
		$res_penalty = sql($sql_penalty);
		$rs_penalty = mysql_fetch_assoc($res_penalty);
		$cnt_penalty = $rs_penalty["cnt"];

		if($rs["hero_old_idx"] && $cnt_penalty == 0) {
			$memo = $rs["mission_title"]." 후기 미등록";
			
			$insPenaltySql   = " INSERT INTO member_penalty (hero_code, type, mission_idx, memo, hero_today, hero_use_yn) VALUES ";
			$insPenaltySql  .= " ('".$rs["hero_code"]."','3','".$rs["hero_old_idx"]."','".$memo." ".$memo_date."',now(),'Y') ";
			$result = sql($insPenaltySql);
			
			//포인트 차감
			adminPoint($rs["hero_code"], '', 'adminUnReview', '', $rs["hero_old_idx"], '', $rs["hero_id"], $rs["mission_title"].' 후기 미등록', $rs["hero_name"], $rs["hero_nick"], $point, "N", 'minus');
		}
	...
	for($i=0; $i<count($_POST["hero_idx"]); $i++) {
		...
		if($rs["hero_old_idx"] && $cnt_penalty > 0) {
			$memo = $rs["mission_title"]." 미등록";
				
			$updPenaltySql   = " UPDATE member_penalty SET hero_use_yn = 'N' ";
			$updPenaltySql  .= " WHERE hero_code = '".$rs["hero_code"]."' AND mission_idx = '".$rs["hero_old_idx"]."' AND type='3' ";
			$result = sql($updPenaltySql);
				
			//포인트 차감
			adminPoint($rs["hero_code"], '', 'adminUnReviewCancel', '', $rs["hero_old_idx"], '', $rs["hero_id"], $rs["mission_title"].' 후기 미등록 취소', $rs["hero_name"], $rs["hero_nick"], $point, "N", '');
		}
...
자세한 내용은 지금 바로 AK LOVER 홈페이지 내 쪽지함을 확인해주세요!";
	...
	for($i=0; $i<count($_POST["hero_idx"]); $i++) {
		...
		$mission_review_res = sql($misison_review_sql);
		...
		if($mission_review_rs["hero_id"]) {
			...
			$sql .= " ) VALUES ( ";
			$sql .= " '".$_SESSION["temp_code"]."', 'mail', '".$message_title."', '".$alrimtalk_type."', '".$mission_review_rs["hero_id"]."' ";
			...
			if($result) {
				...
				adminSendAlrimTalk($alrimMsg,$mission_review_rs["hero_hp"],$type,$hero_idx_mail,$mission_review_rs["hero_id"]);
			}
		}
		if($result) {
			$success_num++;
