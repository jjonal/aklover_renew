...
$sql_gisu = " SELECT hero_life_gisu FROM mission_gisu "; //현재 기수
sql($sql_gisu);
$rs_gisu = mysql_fetch_assoc($out_sql);

$gisu = !$_REQUEST["gisu"] ? $rs_gisu["hero_life_gisu"]:$_REQUEST["gisu"];
...
$list_sql .=  " WHERE g.gisu = '".$gisu."' AND g.hero_board = 'group_04_28' ".$where;
...
function excel() {
	...
	$('#form0').attr('action','nail/excel_08.php').submit();
...
function excelAll() {
	...
	$('#form0').attr('action','nail/excel_08_all.php').submit();
...
<table class="tbForm" style="margin:0 auto;">
	...
	<tr>
		<th><label>라이프클럽 기수</label></th>
		<td>
			<select name="gisu">
				<? for($k=$rs_gisu["hero_life_gisu"]; $k>0; $k--) {?>
...
<div style="margin:30px 0 0 0;">
	<table class="tbForm" style="width:100%; margin:20px 0 0 0;">
		...
		?>
			<tr>
					<td class="c" colspan="9">등록된 데이터가 없습니다.</td>
			</tr>	
		<? } ?>
	</table>
