...
//이달의 Ak Lover선정 추가
if($_POST["type"] == "write") {
    //hero_order max값 구해서 하나씩 올려주기위해
    $sql_sort = "SELECT IFNULL(MAX(hero_order),0) hero_order FROm monthak WHERE hero_old_idx = '".$_GET['hero_old_idx']."' and hero_use = 0";
    $out_res  = @mysql_query($sql_sort);
    ...
    for($i = 0; $i < count($_POST['hero_idx']); $i++) {
        ...
        $sql_list .= " ('".$_SESSION["temp_code"]."','".$_GET['hero_old_idx']."','".$_POST['hero_idx'][$i]."',now(), '".($sort+$i+1)."', '0') ";
        mysql_query ( $sql_list );
    }

    msg ( '등록 되었습니다.', 'location.href="' . PATH_HOME . '?' . get ( 'type', '' ) . '"' );
    exit ();
}
...
sql($sql);
$manage_count = mysql_num_rows($out_sql);
?>

<div class="apply_btn others" onClick="goWrite();">
	저장
</div>
...
<h4>이달의 AK Lover 선정자</h4>

<div class="searchCnt">
    <div class="">
        <span>선정자 : </span>
        <span class="chkCnt chk02"><?=$manage_count?></span>
        <span>/</span>
        <span class="allCnd">20</span>
    </div>
</div>

<div class="searchBoxCho_container">
    <table class="searchBoxCho_head">
        <colgroup>
            <col width="45px" />
            <col width="158px" />
            <col width="105px" />
    ...
    <div class="searchBoxCho_body_scroll">
        <table class="searchBoxCho_body">
            <colgroup>
                <col width="45px" />
                <col width="158px" />
                <col width="105px" />
...
<!--여기서부터 이달의 AK Lover 선정(bestReview.php랑 거의 동일)-->
<?php
//검색폼
$search = "";
...
$total_sql .= " AND b.hero_idx not in (SELECT board_hero_idx FROM monthak WHERE hero_old_idx = '".$_GET['hero_old_idx']."' and hero_use = '0')"; //선정된 후기 제외
...
$sql .= " AND b.hero_idx not in (SELECT board_hero_idx FROM monthak WHERE hero_old_idx = '".$_GET['hero_old_idx']."' and hero_use = '0')"; //선정된 후기 제외
$sql .= $order;
...
<div class="searchCnt">
    ...
    <div class="">
        ...
        <span>|</span>
        <span class="allCnd"><?=20-$manage_count?></span> 
...
<input type="hidden" id="hero_old_idx" name="hero_old_idx" valie="<?=$_GET['hero_old_idx']?>" />
...
<script>
    ...
    //날짜 데이터포맷
    $(function(){
        jQuery("#sdate").AnyTime_picker( {
            format: "%Y-%m-%d %H:%i:00"
        });

        jQuery("#edate").AnyTime_picker( {
            format: "%Y-%m-%d %H:%i:00"
        });
    });
	...
	$('.searchResultBox :checkbox').on('change', function(){
		...
		// 체크된 갯수 표시
		$('.chkCnt').not('.chk02').text($('.searchResultBox :checkbox:checked').not(':checkbox[name="all"]').length);
