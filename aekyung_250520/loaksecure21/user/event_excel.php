...
<? }else { ?>
  <tr align="center">
   ...
   <td valign="middle"><stron>회원상태</strong></td>
  </tr>
<?
	while($list = @mysql_fetch_array($out_sql)){
		$hero_use_txt = "";
		$member_list = "";
		$restCheck = "";
		$sql = "SELECT hero_id, hero_name, hero_nick, hero_hp, hero_address_01, hero_address_02, hero_address_03, hero_mail, hero_use FROM member WHERE hero_code = '".$list['hero_code']."'";
		$member_sql = mysql_query($sql);
		$member_list = @mysql_fetch_array($member_sql);
		
		if($member_list["hero_use"] == 0) {
			$hero_use_txt = "정상";
		} else if($member_list["hero_use"] == 1) {
			$hero_use_txt = "탈퇴";
		} else if($member_list["hero_use"] == 2) {
			$hero_use_txt = "휴면회원";
		}
...
?>
  <tr align="left">
   ...
   <td align="center" valign="middle"><?=$hero_use_txt;?></td>
