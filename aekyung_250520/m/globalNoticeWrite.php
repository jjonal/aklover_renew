...
if($_GET["hero_idx"]) {
	...
	$sql .= " , b.hero_today, b.hero_notice, b.hero_pcMobile, b.hero_ori_file ";
	$sql .= " , b.hero_temp, m.hero_nick, l.hero_img_new ";
...
<div class="writeWrap"> 
<form name="frm" class="form-horizontal" role="form" enctype="multipart/form-data" method="post" action="globalNoticeAction.php?board=<?=$_GET["board"];?>&action=<?=$action?>&idx=<?=$idx;?>&page=<?=$page;?>">
...
<div class="form-group">
	...
	<div class="col-sm-10">		
		...
		<input type="checkbox" name="hero_notice" id="hero_notice" value="1" <?=$view["hero_notice"]==1 ? "checked":"";?> />
		<label for="hero_notice">공지</label>
		<input type="checkbox" name="hero_temp" id="hero_temp" value="1" <?=$view["hero_temp"]==1 ? "checked":"";?>/>
		<label for="hero_temp">임시글</label>
	</div>
</div>
...
function fnList() {
	$("#searchForm").attr("action","globalNoticeList.php").submit();	
}
