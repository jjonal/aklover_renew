<? 
	...
	if($_GET["sns"] != "snsAuth") {//20201207 sns 연동때문에 추가
		if(!strpos($_SERVER["HTTP_REFERER"],"/m/infoauth.php?board=infoauth") && !strpos($_SERVER["HTTP_REFERER"],'/m/infoeditAction.php')) {
			error_location("비밀번호 확인 후 이용해 주세요.","/m/infoauth.php?board=infoauth");
			exit;
		}
	}
	...
	$member_sql .= " , m.area_etc_text, m.hero_chk_phone, m.hero_chk_email ";
...
<div class="clear"></div>
<form name="form_next" id="form_next" method="post" enctype="multipart/form-data">
...
<div class="memberWrap mu_member">
	<div id="subpage" class="mypage mypoint">
		...
		<div class="boardTabMenuWrap">
			<a href="/m/infoedit.php?board=infoedit" class="on">정보 변경</a>
	...
	<div id="infoEditSns">
		<div class="info_wrap join_input">
			<div class="info_box default_info" style="margin-bottom: 3rem;">
				<div class="box_tit fz26 op05">로그인 연동하기</div>
				<div class="cont f_sc">								
					<? if(!$member_list["hero_kakaoTalk"]) {?>
						<div class="info_sns fz24 fw600"><img src="/img/front/icon/edit_kakao.png" alt="카카오톡" onClick="loginKakao('infoedit')">카카오톡 로그인</div>
					<? } else { ?>
						<div class="info_sns fz24 fw600" style="opacity: .5;"><img src="/img/front/icon/edit_kakao.png"  alt="카카오톡" onClick="alert('카카오톡 계정과 이미 연동되었습니다.')"></div>
					<? } ?>
					<? if(!$member_list["hero_naver"]) {?>
						<div class="info_sns fz24 fw600 mu_bar f_c naver_wrap"><div id="naver_id_login"></div>네이버 로그인</div>		
					<? } else { ?>
						<div class="info_sns fz24 fw600 mu_bar" style="opacity: .5;"><img src="/img/front/icon/edit_naver.png" alt="네이버" onClick="alert('네이버 계정과 이미 연동되었습니다.')"></div>
					<? } ?>
					<? if(!$member_list["hero_google"]) {?>
						<div class="info_sns mu_bar fz24 fw600" id="btn_google"><img src="/img/front/icon/edit_google.png" alt="구글 로그인">구글 로그인</div>
					<? } else { ?>
						<div class="info_sns fz24 fw600 mu_bar" style="opacity: .5;"><img src="/img/front/icon/edit_google.png" alt="구글" onClick="alert('구글 계정과 이미 연동되었습니다.')"></div>
					<? } ?>
				</div>			
			</div>			
		</div>
	...
	<div class="info_wrap join_input">
		<div class="info_box default_info">
			<div class="box_tit fz26 op05">기본 입력 정보</div>
			<div class="cont">
				...
				<div class="box_line">
					...
					<div class="fz15 fw500 f_cs mail_select">
						<input type="text" name="hero_mail_01" id="hero_mail_01" value="<?=$hero_mail['0'];?>" style="ime-mode:disabled;"/>
						<span class="txt_sign">@</span>
						<input type="text" name="hero_mail_02" id="hero_mail_02" value="<?=$hero_mail['1'];?>" style="ime-mode:disabled;"/>
						<select id="email_select" name="hero_mail_02_select" onchange="$('#hero_mail_02').val(this.value); $('#hero_mail_02').focus();" class="short">
				...
				<div class="box_line">
					...
					<div class="fz15 fw500 f_cs mail_select">
						<input type="text" name="hero_hp_01" id="hero_hp_01" value="<?=substr($next, '0', '3');?>" onkeyup="if(this.value.length > 2)hero_hp_02.focus();" maxlength="3" numberOnly class="short" />-
						<input type="text" name="hero_hp_02" id="hero_hp_02" value="<?=substr($next, '3', '4');?>" onkeyup="if(this.value.length > 3)chPwdTF(this);" maxlength="4" numberOnly class="short" />-
						<input type="text" name="hero_hp_03" id="hero_hp_03" value="<?=substr($next, '7', '4');?>" onkeyup="if(this.value.length > 3)chPwdTF(this);" maxlength="4" numberOnly class="short" style="width:125px"/>
					</div>
				</div>
				<div class="box_line">
					...
					<div class="fz15 fw500">
						<div class="f_sc" style="align-items: flex-end;">
							...
							<a href="javascript:;" onClick="btnAddressGet()" class="btn_post fz26 fw500">우편번호</a>
						</div>
						<input type="text" name="hero_address_02" id="hero_address_02" value="<?=$member_list['hero_address_02']?>" class="" style="margin-top:10px;"/><br />
						<input type="text" name="hero_address_03" id="hero_address_03" value="<?=$member_list['hero_address_03']?>" class="" style="margin-top:10px;" />
					</div>
				</div>	
			</div>			
		</div>		
	...
	<div class="info_wrap join_input">
		<div class="info_box choice_info">
			<div class="box_tit fz26 op05">선택 입력 정보</div>
			<div class="choice">
				<div class="div_tr">
				...
				<div class="div_tr">
					...
					<div class="div_td">
						...
						<p class="txt_default">각종 이벤트, 체험단, 행사 관련 정보 안내 및<br /> 제반 마케팅 활동, 당사 상품/서비스 안내 및 권유</p>
					</div>
				</div>
				<div class="div_tr">
					...
					<div class="div_td">
						...
						<div class="snsUrl etc_info">
							...
							<dl>
								...
								<!-- <dd><label for="hero_blog_08">기타</label><input type="text" name="hero_blog_08" id="hero_blog_08" class="hero_sns_url" value="<?=$hero_blog_08;?>" <?=$hero_qs_01_disabled?>/></dd> -->
							</dl>
						</div>
						<p class="txt_default" style="margin-top: 12px;">AK Lover는 체험단 활동을 위해서는 개인 SNS URL 입력이 필요합니다.</p>
					</div>
				</div>	
				<div class="div_tr">
					<p class="tit">개인 정보</p>	
					...
					<p class="txt_default" style="margin-top: 12px;">AK Lover는 체험단 활동을 위해서는 개인정보 입력이 필요합니다.</p>	
				</div>		
			</div>	
		</div>
	</div>
</div>
<div class="edit_btn btngroup f_c" >
	<a href="javascript:;" class="btn_submit btn_black" onClick="fnEdit();">정보 수정 하기</a>
</div>	
</form>
<script>
//(s)210203 google 연동
...
<? if(!$member_list['hero_naver']) { ?>
var naver_id_login = new naver_id_login(naver.client_id,"<?=getSnsDomain()?>/m/infoedit.php?sns=snsAuth&board=infoedit");
...
function default_submit(form) {
    ...
    $("#form_next").attr("action","infoeditAction.php").submit();
    return true;
}

function fnEdit() {
	$("input[name='hero_mail_01']").css("border","1px solid #ccc");
	$("input[name='hero_mail_02']").css("border","1px solid #ccc");
	$("#hero_hp_01").css("border","1px solid #ccc");
	$("#hero_hp_02").css("border","1px solid #ccc");
	$("#hero_hp_03").css("border","1px solid #ccc");
	$("#hero_address_01").css("border","1px solid #ccc");
	$("#hero_address_02").css("border","1px solid #ccc");
	$("#hero_address_03").css("border","1px solid #ccc");
	$("#hero_blog_00").css("border","1px solid #ccc");
	$("input[name='hero_qs_08']").css("border","1px solid #ccc");

	if(!$("input[name='hero_mail_01']").val()) {
		alert("이메일(1)을 입력하세요.");
		$("input[name='hero_mail_01']").css("border","1px solid #f00");
	...
	if(!$("input[name='hero_mail_02']").val()) {
		alert("이메일(2)을 입력하세요.");
		$("input[name='hero_mail_02']").css("border","1px solid #f00");
	...
	if(!$("#hero_hp_01").val()) {
		...
		$("#hero_hp_01").css("border","1px solid #f00");
		$("#hero_hp_01").focus();
		return;
	}
	...
	if(!$("#hero_hp_02").val()) {
		...
		$("#hero_hp_02").css("border","1px solid #f00");
		$("#hero_hp_02").focus();
		return;
	}
	...
	if(!$("#hero_hp_03").val()) {
		...
		$("#hero_hp_03").css("border","1px solid #f00");
		$("#hero_hp_03").focus();
		return;
	}
	...
	if(!$("#hero_address_01").val()) {
		alert("우편번호를 입력하세요.");
		$("#hero_address_01").css("border","1px solid #f00");
	...
	if(!$("#hero_address_02").val()) {
		alert("주소를 입력하세요.");
		$("#hero_address_02").css("border","1px solid #f00");
	...
	if(!$("#hero_address_03").val()) {
		alert("상세주소를 입력하세요.");
		$("#hero_address_03").css("border","1px solid #f00");
