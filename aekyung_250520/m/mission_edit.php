...
$sql = " SELECT * FROM mission WHERE hero_idx='".$_GET['mission_idx']."' ";
sql($sql, 'on');
...
$right_sql = " SELECT * FROM hero_group WHERE hero_order!='0' AND hero_use='1' AND hero_board ='".$_GET['board']."' ";
$out_right_sql = @mysql_query($right_sql);
$right_list = @mysql_fetch_assoc($out_right_sql);

//관리자가 수정 할 경우(체험후기에서)
if($_GET['somun'] == 'Y') {
	$where = "and hero_old_idx = '".$_GET['mission_idx']."' and hero_idx = '".$_GET['hero_idx']."'";
	$where2 = "and hero_idx = '".$_GET['board_idx']."'";
}else {
	$where = "and hero_old_idx = '".$_GET['mission_idx']."' and hero_code = '".$_SESSION['temp_code']."'";
...
if($mission_board_type) {
	...
	$board_sql .= " WHERE hero_01=".$_GET['mission_idx']." ".$where2."";
...
$sql_survey .= " WHERE s.mission_idx = '".$_GET['mission_idx']."'  ORDER BY s.order_num ASC ";
...
if(!strcmp($_GET['type'], 'edit')){
	...
	if($mission_board_type) { //소문내기일때 다른 프로세스 탐
		$url = array();
		$gubun = array();
		$member_check = array();
	...
	msg('수정 되었습니다.','location.href="'.DOMAIN_END.'m/mission_completion.php?'.get('type').'"');
	exit;
}
...
if(!strcmp($_GET['type'], 'drop')){
	...
	//배송피 포인트 삭제
	if($out_row["delivery_point_yn"] == "Y" && $edit_list["delivery_point_yn"] == "Y") {
	...
	if($mission_board_type) {	
		$sel_board_sql = " SELECT hero_idx FROM board WHERE hero_code = '".$_SESSION['temp_code']."' AND hero_01 = '".$_GET['mission_idx']."' AND hero_table = '".$_GET['board']."' ";
		$sel_board_res = sql($sel_board_sql);
		$sel_board_rs = mysql_fetch_assoc($sel_board_res);
		...
		//URL 삭제
		$del_mission_url_sql = " DELETE FROM mission_url WHERE board_hero_idx = '".$sel_board_rs["hero_idx"]."' ";
		sql($del_mission_url_sql);
	}
...
<div id="content">
	...
	<!-- (e) 인트로 문구 -->
	<div id="application_title"><span class="star_icon">l </span> 체험단 신청자</div>
	
	<form name="form_next" action="<?=DOMAIN_END.'m/mission_edit.php?'.get('','type=edit');?>" method="post" enctype="multipart/form-data" onSubmit="return false;"> 
	...
	<div id="board" class="apply_wrap">
		<? if($mission_board_type == false && $out_row['hero_question_url_yn'] != "N"){ ?>
		<div class="answerWrap">
			<p class="tit">URL을 입력해주세요.<br/>
				<? if($out_row['hero_question_url_check'] == "1") {?>
					<span class="txt_ex_orange">* 네이버 블로그 URL은 필수로 입력하셔야 합니다.</span>
				<? } else if($out_row['hero_question_url_check'] == "2") {?>
					<span class="txt_ex_orange">* 인스타그램 URL은 필수로 입력하셔야 합니다.</span>
				<? } else if($out_row['hero_question_url_check'] == "3") {?>
					<span class="txt_ex_orange">* 네이버 블로그/인스타그램 URL 중 한개의 URL은 필수로 입력하셔야 합니다.</span>
				<? } else if($out_row['hero_question_url_check'] == "4") {?>
					<span class="txt_ex_orange">* 네이버 블로그, 인스타그램 URL은 필수로 입력하셔야 합니다.</span>
				<? } else if($out_row['hero_question_url_check'] == "5") {?>
					<span class="txt_ex_orange">* 영상 채널(유튜브, 네이버TV, 틱톡, 기타 등) URL은 필수로 입력하셔야 합니다.</span>
				<? } else if($out_row['hero_question_url_check'] == "6") {?>
					<span class="txt_ex_orange">* 네이버 블로그/인스타그램/영상 채널(유튜브, 네이버TV, 틱톡, 기타 등) URL중 한개의 URL은 필수로 입력하셔야 합니다.</span>
				<? } ?>
			</p>
			<? 
				...
				for($i=0; $i<count($blog_url); $i++) {
					switch ($hero_question_url_list[$i]) {
						case "블로그":
							...
							?>
                			<dl class="answer_dl">
                				<dt><?=$hero_question_url_list[$i]=="블로그" ? "네이버 블로그":$hero_question_url_list[$i]?> URL</dt>
                				<dd>&nbsp;&nbsp;https://blog.naver.com/<input type="text" id="hero_blog_0<?=$i?>" name="<?=$blog_id?>" class="hero_blog_cnt hero_blog form-control w100p" value="<?=str_replace("https://blog.naver.com/", "", $blog_url[$i]);?>" placeholder="네이버 또는 블로그 ID 입력" style="width:51%;display:inline;"></dd>
                			</dl>
                			<?
							break;
						...
						case "인스타그램":
                			...
                			<dl class="answer_dl">
                				<dt><?=$hero_question_url_list[$i]=="블로그" ? "네이버 블로그":$hero_question_url_list[$i]?> URL</dt>
                				<dd>&nbsp;&nbsp;https://www.instagram.com/<input type="text" id="hero_blog_0<?=$i?>" name="<?=$blog_id?>" class="hero_blog_cnt hero_blog form-control w100p" value="<?=str_replace("https://www.instagram.com/", "", $blog_url[$i]);?>" placeholder="인스타그램 ID 입력" style="width:42%;display:inline;"></dd>
                			</dl>
                			<?
							break;
						...
						case "페이스북":
                			...
                			<dl class="answer_dl">
                				<dt><?=$hero_question_url_list[$i]=="블로그" ? "네이버 블로그":$hero_question_url_list[$i]?> URL</dt>
                				<dd><input type="text" id="hero_blog_0<?=$i?>" name="<?=$blog_id?>" class="hero_blog_cnt hero_blog form-control w100p" value="<?=$blog_url[$i]?>" placeholder="http:// 또는 https:// 포함된 SNS URL을 입력해주세요."></dd>
                			</dl>
                			<?
							break;
		...
		<?
			...
			<div class="answer_box">
		    	<p class="tit">
		    		<?if($row_survey["necessary"] == "Y") {?><span class="star_icon">*</span> <? } ?>
		...
		<? if($mission_board_type){ //소문내기?>
		<div class="missionBoardWrite line-bottom">
			<p><span class="txt_emphasis">*</span>는 필수 입력 항목입니다!!!</p>
			
			<dl class="writeForm">
    			<dt><span class="star_icon">*</span> <label>제목</label></dt>
    			<dd><input type="text" name="hero_title" id="hero_title" value="<?=$board_list['hero_title']?>"/></dd>
    			<dt><label>작성자</label></dt>
    			<dd><?=$_SESSION['temp_nick'];?></dd>
    			<dt><span class="star_icon">*</span>  <label>대표이미지</label></dt>
	    		<dd>
	    		...
	    		<dd>
	    			<input type="text" name="naver_url" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl" value="<?=$naver_rs["url"]?>"/>
	    		...
	    		<dd>
					...
					<dl class="urlAgreeBox">
						...
						<dd><input type="radio" name="insta_member_check" id="insta_member_check_Y" value="Y" <?=$insta_rs["member_check"] == "Y" ? "checked":"";?>/><label for="insta_member_check_Y">예</label>
							<input type="radio" name="insta_member_check" id="insta_member_check_N" vlaue="N" <?=$insta_rs["member_check"] == "N" ? "checked":"";?>/><label for="insta_member_check_N">아니오</label>
						</dd>
					</dl>
					<p class="txt_agree_info mgb10">
						※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK LOVER 활동에 불이익이 있을 수 있습니다.
	    		...
	    		<dt><label>후기(영상)</label></dt>
	    		<dd>
	    			<div class="ui_urlBox">
	    				<div class="ui_url">
			    			<input type="text" name="movie_url[]" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl3"/>
							...
							<dl class="urlAgreeBox">
								...
								<dd><input type="radio" name="movie_member_check1" id="movie_member_check1_Y" value="Y"/><label for="movie_member_check1_Y">예</label>
									<input type="radio" name="movie_member_check1" id="movie_member_check1_N" vlaue="N"/><label for="movie_member_check1_N">아니오</label>
								</dd>
							</dl>
							<p class="txt_emphasis mgb10">
								※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK LOVER 활동에 불이익이 있을 수 있습니다.
							</p>
							<? 
							...
							?>
								<div class="ui_url">
									<input type="text" name="movie_url[]" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl3" value="<?=$movie_list["url"]?>"/>
	    		...
	    		<? } ?>
	    		<dt><label>카페</label></dt>
	    		<dd>
	    			<div class="ui_urlBox">
	    				<div class="ui_url">
			    			<input type="text" name="cafe_url[]" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl3" value="<?=$cafe_rs["url"]?>"/>
							<a href="javascript:;" onClick="fnUrl(this,'add')" class="btn_url_add">+</a> 
							<dl class="urlAgreeBox">
								<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
								<dd><input type="radio" name="cafe_member_check1" id="cafe_member_check1_Y" value="Y" <?=$cafe_rs["member_check"] == "Y" ? "checked":"";?>/><label for="cafe_member_check1_Y">예</label>
									<input type="radio" name="cafe_member_check1" id="cafe_member_check1_N" vlaue="N" <?=$cafe_rs["member_check"] == "N" ? "checked":"";?>/><label for="cafe_member_check1_N">아니오</label>
								</dd>
							</dl>
							<p class="txt_agree_info mgb10">
								※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK LOVER 활동에 불이익이 있을 수 있습니다.
						...
						?>
							<div class="ui_url">
								<input type="text" name="cafe_url[]" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl3" value="<?=$cafe_list["url"]?>"/>
								<a href="javascript:;" onClick="fnUrl(this,'minus')" class="btn_url_minus">-</a> 	
								<dl class="urlAgreeBox">
									<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
	    		...
	    		<dd>
	    			<div class="ui_urlBox">
	    				<div class="ui_url">
			    			<input type="text" name="etc_url[]" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl3" value="<?=$etc_rs["url"]?>"/>
							<a href="javascript:;" onClick="fnUrl(this,'add')" class="btn_url_add">+</a> 
							<dl class="urlAgreeBox">
								<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
								<dd><input type="radio" name="etc_member_check1" id="etc_member_check1_Y" value="Y" <?=$etc_rs["member_check"] == "Y" ? "checked":"";?>/><label for="naver_member_check1_Y">예</label>
									<input type="radio" name="etc_member_check1" id="etc_member_check1_N" vlaue="N" <?=$etc_rs["member_check"] == "N" ? "checked":"";?>/><label for="naver_member_check1_N">아니오</label>
								</dd>
							</dl>
							<p class="txt_agree_info mgb10">
								※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK LOVER 활동에 불이익이 있을 수 있습니다.
						...
						?>
							<div class="ui_url">
								<input type="text" name="etc_url[]" placeholder="반드시 포스팅 URL을 입력해주세요." class="inputUrl3" value="<?=$etc_list["url"]?>"/>
								<a href="javascript:;" onClick="fnUrl(this,'minus')" class="btn_url_minus">-</a> 	
								<dl class="urlAgreeBox">
									<dt>※ 공정거래위원회 문구를 작성하였습니까?</dt>
									<dd><input type="radio" name="etc_member_check<?=$k?>" id="etc_member_check<?=$k?>_Y" value="Y" <?=$etc_list["member_check"] == "Y" ? "checked":"";?>/><label for="cafe_member_check<?=$k?>_Y">예</label>
										<input type="radio" name="etc_member_check<?=$k?>" id="etc_member_check<?=$k?>_N" value="N" <?=$etc_list["member_check"] == "N" ? "checked":"";?>/><label for="cafe_member_check<?=$k?>_N">아니오</label>
									</dd>
								</dl>
								<p class="txt_agree_info mgb10">
									※ 공정거래위원회의 ‘추천보증 등에 관한 표시광고 심사지침’ 또는 ‘전자상거래 등에서의 소비자보호에 관한 법률’에 의한 필수 기재사항으로, 작성되지 않을 경우 AK LOVER 활동에 불이익이 있을 수 있습니다.
	    ...
	    <div class="answerWrap">
	    	...
	    	<dl class="answer_dl">
				<dt>받으시는분</dt>
				<dd><input type="text" name="hero_new_name" id="hero_new_name" class="input1 form-control mb10" value="<?=$edit_list['hero_new_name']?>" /></dd>
				<dt>연락처</dt>
				<dd>
					<input type="text" name="hero_hp_01" id="hero_hp_01" class="input1 form-control" value="<?=$edit_list['hero_hp_01'];?>" style="width:29%;float:left;margin: 0 4% 2% 0;" value="<?echo (substr($next, '0', '3')? substr($next, '0', '3') : "010");?>" onKeyUp="if(this.value.length >= 3)hero_hp_02.focus();" maxlength="3"/>	
					<input type="text" name="hero_hp_02" id="hero_hp_02" class="input1 form-control" value="<?=$edit_list['hero_hp_02'];?>" style="width:29%;float:left;margin: 0 4% 2% 0;" value="<?=substr($next, '3', '4');?>" onKeyUp="if(this.value.length >= 4)hero_hp_03.focus();" maxlength="4"/> 
					<input type="text" name="hero_hp_03" id="hero_hp_03" class="input1 form-control" value="<?=$edit_list['hero_hp_03'];?>" style="width:29%;float:left;margin: 0 4% 2% 0;" value="<?=substr($next, '7', '4');?>" maxlength="4"/>
				</dd>
				<? if($out_row['hero_type'] != 8) { ?>
				<dt>배송지주소</dt>
				<dd>
					<input type="text" name="hero_address_01" id="hero_address_01" class="input form-control zipCodeInput" value="<?=$edit_list['hero_address_01']?>" value="<?=$site_list['hero_address_01']?>" onClick="javascript:btnAddressGet()" readonly/>
					<a href="javascript:btnAddressGet()"><img src="img/general/address_btn.jpg" alt="우편번호" width="75px"/></a>
					<div class="mgt10"><input type="text" name="hero_address_02" id="hero_address_02" class="input form-control" onClick="javascript:btnAddressGet()" value="<?=$edit_list['hero_address_02']?>" readonly/></div>
					<div class="mgt10"><input type="text" name="hero_address_03" id="hero_address_03" class="input form-control" value="<?=$edit_list['hero_address_03']?>"/></div>
				</dd>
				<? } else { ?>
	...
	<div class="apply_btn_wrap_c">
		<a href="javascript:" class="m_content_btn" onClick="go_submit(document.form_next);">수정</a>
		<a href="javascript:" class="m_content_btn" onClick="location.href='<?=DOMAIN_END.'m/mission_edit.php?'.get('type||hero_idx','type=drop&hero_idx='.$_REQUEST['hero_idx'])?>'">삭제</a>
		<a href="javascript:" class="m_content_btn" onClick="location.href='mission.php?<?=get('page||hero_idx||mission_idx')?>'">취소</a>
	</div>
	</form>
</div> 
...
$(document).ready(function(){
    ...
    function go_submit(form) {
        ...
        //var hero_01 = form.hero_01; 용도 모름
