...
if(!strcmp($_SESSION['temp_level'], '') || !$_GET['action'] || !$_SESSION["temp_code"]){
	error_location('비정상적인 접근입니다. 로그인 후에 다시 시도해 주세요.', '/m/main.php');exit;
}
...
#변수설정
	...
	$idx 			=		$_GET['hero_idx'];
	$mission_idx    = 		$_GET['idx']; //이전 페이지에서는 idx가 미션번호
	$action			=		$_GET['action'];
	$board			=		$_GET['board'];
	$page			=		$_GET['page'];
...
## 본문내용 처리
	...
	######################################################################################################################################################
		...
		if($check_rs['hero_idx']){
			$href = "/m/mission_view.php?board=".$check_rs['hero_table']."&page=1&mission_idx=".$mission_idx;
		...
		if(!$out_idx_sql){
			logging_error($my_nick, $board."-[mobile] MISSION_ACTION_03_07", $full_today);
			error_historyBack("");
		}
		...
		$pf = mysql_query($sql);
		if(!$pf){
			logging_error($my_nick, $board."-[mobile] MISSION_ACTION_03_08", $full_today);
			error_historyBack("");
			exit;
		}
		...
		if($point_rs!=1){
			...
			$pf_rollback = mysql_query($rollback_query);
				if(!$pf_rollback){
					logging_error($my_nick, $board."-[mobile] MISSION_ACTION_03_09", $full_today);
				}
			error_historyBack("");
			exit; 
		}
		...
		pointDel($idx,$hero_table,"mission_write");
		...
		$mission_url_drop_sql = " DELETE FROM mission_url WHERE board_hero_idx = '".$idx."' ";
	...
	if($_GET["board"] == "group_04_05" && $_GET["action"] == "update" && $_GET["pre_board"] != "group_04_09") {
		$get_herf = "https://".$_SERVER["HTTP_HOST"]."/m/board_view_01.php?board=".$_GET["board"]."&page=".$page."&hero_idx=".$idx."&idx=".$_GET["idx"];
	} else {
		$get_herf = "https://".$_SERVER["HTTP_HOST"]."/m/board_01.php?board=".$_GET["board"]."&page=".$page."&idx=".$mission_idx;
	}
