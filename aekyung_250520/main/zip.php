...
if(!strcmp($_POST['type'],'zip')){
    if(strcmp($_POST['input_chat'],'')){
        ...
        while($list  = @mysql_fetch_assoc($out_sql)){ 
            $out .= '<div style="cursor:pointer;" onclick="fnLoad_01(\''.$list['zipcode'].'\', \''.$list['sido'].'\', \''.$list['gugun'].'\', \''.$list['dong'].'\', \''.$list['bunji'].'\', \''.$list['seq'].'\')">'.$list['gugun'].' '.$list['dong'].' '.$list['bunji'].'</div><br>';//.' '.$list['seq']
        }
        echo iconv('EUC-KR', 'UTF-8', $out);
    }
}else if(!strcmp($_POST['type'],'id')){
	
	include_once $_SERVER['DOCUMENT_ROOT']."/classGathered/chTextTypeClass.php";
	$chTextType = new chTextType($_POST['input_chat']);
	$typeResult = $chTextType->is_kr();
	if($typeResult==true){
		$out= "<font style='color:".$_MAIN_COLOR[0].";'>한글은 사용하실 수 없습니다</font><input type=hidden id='id_action' value='hero_down'>";
		//echo $out;
		echo getIconv($out);
		exit;
	}
	
	$typeResult = $chTextType->is_other();
	if($typeResult==true){
		$out= "<font style='color:".$_MAIN_COLOR[0].";'>특수문자는 사용하실 수 없습니다</font><input type=hidden id='id_action' value='hero_down'>";
		//echo $out;
		echo getIconv($out);
		exit;
	}
    ...
    if($str_len < 6 || $str_len > 20){
    	$out = "<font style='color:".$_MAIN_COLOR[0].";'>이미 사용 중인 아이디입니다</font>";
    	$out .= '<input type=hidden id="id_action" value="hero_down">';
    	//echo $out;
    	echo getIconv($out);
    	exit;
    }
    ...
    if(!strcmp($count,'0')){
		...
		//################### 휴면계정 추가 ######################
		$sql = "select * from member_backup where hero_id = '".$_POST['input_chat']."' ";
		$sql = out($sql);
		sql($sql, 'end');
		$count = @mysql_num_rows($out_sql);
		...
		if($count == 0){
			...
			$out .= '<input type=hidden id="id_action" value="hero_ok">';			
		}else{
			$out = "<font style='color:".$_MAIN_COLOR[0].";'>이미 사용 중인 아이디입니다.</font>";
			$out .= '<input type=hidden id="id_action" value="hero_down">';
		}
    ...
    if($str_len < 4 || $str_len > 21){
    	$out = "<font style='color:".$_MAIN_COLOR[0].";'>닉네임 글자 수를 확인해주세요.</font>";
    ...
    if(!strcmp($count,'0')){
		...
		//################### 휴면계정 추가 ######################
		$sql = "select * from member_backup where hero_nick = '".$_POST['input_chat']."' ";
		$sql = out($sql);
		sql($sql, 'end');
		$count = @mysql_num_rows($out_sql);
    ...
    if(!strcmp($_POST['mode'],'review_write')){
        $sql = 'select * from board where hero_idx = \''.$_GET['hero_idx'].'\';';
        sql($sql, 'on');
        ...
        $sql_two_write = '\''.$good_idx.'\', \''.$_SESSION['temp_code'].'\', \''.$review_list['hero_table'].'\', \''.$_GET['hero_idx'].'\', \''.$_SESSION['temp_name'].'\', \''.Ymdhis.'\', \''.iconv('UTF-8', 'EUC-KR', $_POST['input_chat']).'\'';
        $sql = 'INSERT INTO review ('.$sql_one_write.') VALUES ('.$sql_two_write.');';
        @mysql_query($sql);
        $sql = 'UPDATE board SET hero_review_count=\''.$review_count.'\' WHERE hero_idx = \''.$_GET['hero_idx'].'\';';
        @mysql_query($sql);
...
######################################################################################################################################################
        ...
        if( (!strcmp($my_level, '0')) or (!strcmp($temp_point, '0')) ){
            ...
            $sql = 'select * from point where hero_table=\''.$review_list['hero_table'].'\' and hero_code = \''.$temp_code.'\' and hero_type=\''.$_POST['type'].'\' and hero_old_idx=\''.$_GET['hero_idx'].'\' order by hero_today desc limit 0,1;';
            sql($sql, 'on');
...
######################################################################################################################################################
                        ...
                        if(!strcmp($level_up_ok, '0')){
                            ...
                            $sql_two_write = '\''.$good_idx.'\', \''.$_GET['hero_idx'].'\', \''.$temp_code.'\', \''.$review_list['hero_table'].'\', \''.$_POST['type'].'\', \''.$temp_id.'\', \''.$temp_top_title.'\', \''.$temp_title.'\', \''.$_SESSION['temp_name'].'\', \''.$temp_point.'\', \''.Ymdhis.'\'';
...
######################################################################################################################################################
                    ...
                    $sql_one_write = 'hero_review_idx, hero_old_idx, hero_code, hero_table, hero_type, hero_id, hero_top_title, hero_title, hero_name, hero_point, hero_today';
                    $sql_two_write = '\''.$good_idx.'\', \''.$_GET['hero_idx'].'\', \''.$temp_code.'\', \''.$review_list['hero_table'].'\', \''.$_POST['type'].'\', \''.$temp_id.'\', \''.$temp_top_title.'\', \''.$temp_title.'(당일 포인트 초과)\', \''.$_SESSION['temp_name'].'\', \'0\', \''.Ymdhis.'\'';
        //            $sql = 'INSERT INTO point ('.$sql_one_write.') VALUES ('.$sql_two_write.');';
        //            mysql_query($sql);
                }
######################################################################################################################################################
...
//        sql($sql, 'on');
    ...
    $drop_sql = 'SELECT * FROM point WHERE hero_code=\''.$drop_sql['hero_code'].'\' and hero_table=\''.$drop_sql['hero_table'].'\' and hero_review_idx = \''.$_GET['hero_idx'].'\';';
    ...
    sql($sql);
    $sql = 'DELETE FROM point WHERE hero_review_idx = \''.$_GET['hero_idx'].'\';';
    sql($sql);
    $sql = 'DELETE FROM review WHERE hero_idx = \''.$_GET['hero_idx'].'\';';
    sql($sql);

}else if(!strcmp($_POST['type'],'review_depth')){

    $out = $_GET['hero_idx']."ddr";
