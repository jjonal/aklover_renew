...
if(!strcmp($_POST['type'],'id')){ //아이디 찾기
    ...
    if(!strcmp($count,'0')){
		...
		//################### 휴면계정 추가 ######################
		$sql = 'select * from member_backup where hero_name = \''.$_POST['01'].'\' and hero_jumin=\''.$_POST['02'].'\' and hero_mail=\''.$_POST['03'].'\' and hero_use=0';
		$sql = out($sql);
		sql($sql, 'end');
		$count = @mysql_num_rows($out_sql);
    ...
    echo iconv('CP949', 'UTF-8', $out);
} else if(!strcmp($_POST['type'],'pw')){ //확인불가
    $str_len = strlen($_POST['01']);
    if(($str_len > '3') and ($str_len < '21') ){
        $sql = "select * from member where hero_id = '".$_POST['01']."' and hero_jumin='".$_POST['02']."' and hero_mail='".$_POST['03']."'";
        $sql = out($sql);
        sql($sql, 'end');
        $count = @mysql_num_rows($out_sql);
        $review_list                             = @mysql_fetch_assoc($out_sql);
            if(!strcmp($count,'0')){
				...
				//################### 휴면계정 추가 ######################
				$sql = "select * from member_backup where hero_id = '".$_POST['01']."' and hero_jumin='".$_POST['02']."' and hero_mail='".$_POST['03']."'";
				$sql = out($sql);
				sql($sql, 'end');
				$count = @mysql_num_rows($out_sql);
				...
				if($count == 0){
					...
					$sql = "update member_backup set hero_pw='".$pw_sha3_256."' where hero_id = '".$_POST['01']."' and hero_jumin='".$_POST['02']."' and hero_mail='".$_POST['03']."' and hero_code='".$review_list["hero_code"]."'";
					$sql = out($sql);
					sql($sql, 'end');
						...
						<div style='border:3px solid #acacac;width:600px;' align='center'>
							...
							<div style='width:80%;text-align:center;' align='center'>
								...
								<div style='width:100%;text-align:left;color:#666666;font-size:13px;padding-top:15px;padding-bottom:50px;' align='left'>
									...
									<a href='".DOMAIN_END."main/index.php?board=pwreset&auth=".$authCode."&id=".$target_id."' style='color:#000000;' target='_blank'>
									".DOMAIN_END."main/index.php?board=pwreset&auth=".$authCode."&id=".$target_id."
									</a>
...
} else if(!strcmp($_POST['type'],'pw2')){ //비밀번호 찾기
    ...
    if(($str_len > '3') and ($str_len < '21') ){
        ...
        if(strpos($sql_list['SEND_MESSAGE'] , $_POST['auth']) == false){
            //문자 전송?으면 LOG에서 확인
            ...
            if(strpos($sql_log_list['SEND_MESSAGE'] , $_POST['auth']) == false){
                $out = "일치하는 회원정보가 없습니다. 입력하신 정보를 확인하여 주세요.";
                echo iconv('CP949', 'UTF-8', $out);
                exit;
            }
        ...
        $sql = " SELECT * FROM member where hero_id = '".$_POST['hero_id']."' AND hero_name = '".$_POST['hero_name']."' AND hero_hp='".$_POST['hero_hp']."' AND hero_use = 0 ";
        $sql = out($sql);
        sql($sql, 'end');
        $count = @mysql_num_rows($out_sql);
        $review_list                             = @mysql_fetch_assoc($out_sql);
        if(!strcmp($count,'0')){
            //################### 휴면계정 추가 ######################
            $sql = "select * from member_backup where hero_id = '".$_POST['hero_id']."' and hero_name='".$_POST['hero_name']."' and hero_hp='".$_POST['hero_hp']."'";
            $sql = out($sql);
            sql($sql, 'end');
            $count = @mysql_num_rows($out_sql);
            $review_list                             = @mysql_fetch_assoc($out_sql);
...
} else if(!strcmp($_POST['type'],'sendAuth')){ //인증코드 받기
    $str_len = strlen($_POST['hero_id']);
    if(($str_len > '3') and ($str_len < '21') ){
        $sql = " SELECT * FROM member where hero_id = '".$_POST['hero_id']."' AND hero_name = '".$_POST['hero_name']."' AND hero_hp='".$_POST['hero_hp']."' AND hero_use = 0 ";
        $sql = out($sql);
