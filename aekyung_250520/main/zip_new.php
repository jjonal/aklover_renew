...
######################################################################################################################################################
    ...
    if( ($point_total_point>=$today_user_total) and ($point_rev_point>=$today_user_rev) ){
        ...
        $sql_two_write = '\''.$auto_idx.'\', \''.$_REQUEST['board_idx'].'\', \''.$_SESSION['temp_code'].'\', \''.$group_table_name.'\', \''.$_POST['type'].'\', \''.$_SESSION['temp_id'].'\', \''.$group_list['hero_title'].'\', \''.$board_list['hero_title'].'\', \''.$_SESSION['temp_name'].'\', \''.$group_list['hero_rev_point'].'\', \''.Ymdhis.'\'';
...
######################################################################################################################################################
}else if(!strcmp($_REQUEST['mode'],'review_edit')){
######################################################################################################################################################
    $sql_one_update = 'hero_command=\''.iconv('UTF-8', 'EUC-KR', $_POST['input_chat']).'\'';
    $sql = 'UPDATE review SET '.$sql_one_update.' WHERE hero_idx = \''.$_REQUEST['save_id'].'\';';
    @mysql_query($sql);
######################################################################################################################################################
}
