...
if($_SESSION['temp_level']<9999){
	...
	echo 'message:비정상적인 접근입니다. 로그인 후에 다시 시도해 주세요.';
	exit;
...
if($mission_idx){
	...
	if($main_rs_count==0){
		$event_data .= "<tr>";
		$event_data .= "<td colspan='5'>데이터가 없습니다.</td>";
		$event_data .= "</tr>";
	}else{
		while($main_rs = mysql_fetch_assoc($main_res)){
			...
			$event_data .= "<tr>";
			$event_data .= "<td><input type='checkbox' value='".$main_rs['hero_idx']."'/></td>";
